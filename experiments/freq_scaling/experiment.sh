            ...
            do
                {
                    ...
                    python ../../measurement.py ${bluetooth_addr} $((2 * $interval * $amount_jobs)) 1 > ./measurement_data/$x-$size-$interval-iter$i.csv &
                    measurement_process=$!
                    # Start the simulation on the RPi
                    ssh ${user}@${host} "cd /home/$user/freq_experiment/ && ./start_simulation.sh $interval $amount_jobs $(($size * 1000000)) ./data/$x-$size-$interval-iter$i.csv"
                    ...
                    echo "$x-$size-$interval-iter$i.csv" > /dev/stderr
                }
                sleep 10
            done
        done
