...
# ANOVA
d1 <- data.frame("energy" = p1_energy, "sec" = rep(1, length(p1_energy)))
d2 <- data.frame("energy" = p2_energy, "sec" = rep(5, length(p2_energy)))
d3 <- data.frame("energy" = p3_energy, "sec" = rep(10, length(p3_energy)))
d4 <- data.frame("energy" = p4_energy, "sec" = rep(15, length(p4_energy)))
d <- rbind(d1, d2, d3, d4)
a <- aov(energy ~ sec, d)
summary(a)
